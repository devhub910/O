...
# دالة للتحقق مما إذا كان الخادم قابلًا للهجوم
def check_server(ip, port):
    try:
        ...
        print(Fore.GREEN + f"[+] Server at {ip}:{port} is reachable! You can attack.")
        sock.close()
        return True
    except Exception as e:
        print(Fore.RED + f"[!] Error: Server at {ip}:{port} is not reachable. {e}")
        return False
...
if __name__ == "__main__":
    ...
    if check_server(ip, port):
        ...
        print(Fore.RED + "[!] Cannot start attack, server is not reachable.")